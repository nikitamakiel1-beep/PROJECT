...
import type { RuntimeEnv as CoreRuntimeEnv } from "../../creixement-runtime/src/env.js";
...
function effectiveEnv(env: RuntimeEnv): RuntimeEnv {
  ...
  return {
    ...
    CREIXEMENT_RUNTIME_VERSION: env.CREIXEMENT_RUNTIME_VERSION?.trim() || "1.0.0",
...
function requiredBinding(value: string | undefined, name: string): string {
  ...
  if (!normalized) throw new Error(`Missing required scheduler binding: ${name}`);
  return normalized;
}

function optionalHttps(value: string | undefined, name: string): string | null {
  const normalized = value?.trim().replace(/\/$/, "") || null;
  if (normalized && !normalized.startsWith("https://")) throw new Error(`${name} must use HTTPS`);
  return normalized;
}

function scheduledRuntimeEnv(env: RuntimeEnv): CoreRuntimeEnv {
  const supabaseUrl = requiredBinding(env.SUPABASE_URL, "SUPABASE_URL").replace(/\/$/, "");
  if (!supabaseUrl.startsWith("https://")) throw new Error("SUPABASE_URL must use HTTPS");
  const relayUrl = optionalHttps(env.CREIXEMENT_DB_RELAY_URL, "CREIXEMENT_DB_RELAY_URL");
  const timeout = Number(env.CREIXEMENT_DB_TIMEOUT_MS ?? "8000");
  return {
    supabaseUrl,
    ...
    requestTimeoutMs: Number.isInteger(timeout) && timeout >= 1000 && timeout <= 30000 ? timeout : 8000,
  };
}

async function runScheduledTick(env: RuntimeEnv, controller: ScheduledController): Promise<void> {
  const runtime = scheduledRuntimeEnv(env);
  ...
  console.log(JSON.stringify({
    ...
    source: `cloudflare-cron:${controller.cron}`,
    dataPlane: runtime.databaseRelayUrl ? "managed-relay" : "direct-supabase",
    ok: result.ok,
...
function statusPage(env: RuntimeEnv): Response {
  const runtimeId = escapeHtml(env.CREIXEMENT_RUNTIME_ID ?? "kairon-cloudflare-v10");
  const version = escapeHtml(env.CREIXEMENT_RUNTIME_VERSION ?? "1.0.0");
  ...
  const dataPlane = escapeHtml(env.CREIXEMENT_DB_RELAY_URL?.trim() ? "managed database relay" : "direct Supabase");
...
export default {
  ...
  scheduled(controller: ScheduledController, env: RuntimeEnv, ctx: ExecutionContext): void {
    const resolved = effectiveEnv(env);
    ctx.waitUntil(runScheduledTick(resolved, controller).catch((error: unknown) => {
