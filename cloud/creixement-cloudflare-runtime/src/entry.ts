...
type RuntimeEnv = Parameters<typeof runtimeWorker.fetch>[1] & {
...
function statusPage(env: RuntimeEnv): Response {
  const runtimeId = escapeHtml(env.CREIXEMENT_RUNTIME_ID ?? "kairon-cloudflare-v8");
  const version = escapeHtml(env.CREIXEMENT_RUNTIME_VERSION ?? "0.8.0");
...
export default {
  fetch(request: Request, env: RuntimeEnv): Promise<Response> | Response {
