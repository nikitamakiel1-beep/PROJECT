...
const AUTOMATIC_L2_TRUTH_FLOOR = KAIRON_TRUTH_STRENGTH.governed_source_evidence;
...
function truthAllowsAutomaticL2(level: KaironTruthLevel): boolean {
  return KAIRON_TRUTH_STRENGTH[level] >= AUTOMATIC_L2_TRUTH_FLOOR;
}

function automaticL2Evidence(evidence: KaironEvidenceSignal[]): KaironEvidenceSignal[] {
  return evidence.filter((signal) => truthAllowsAutomaticL2(signal.truthLevel));
}
...
function compileEconomicGate(input: KaironSupervisorInput, woundPriority: number, executionMultiplier: number) {
  ...
  const reason = enabled
    ...
    : !canary.promotable
      ? canary.reason
      : !input.economicEvidenceReady
        ? "governed economic evidence is not ready"
        : woundPriority >= 0.8
          ? "runtime injury forces recovery before economic L2"
          : `hysteresis gate is ${hysteresis.state}; dwell threshold is not yet satisfied`;
...
export function compileKaironSupervisorCycle(input: KaironSupervisorInput): KaironSupervisorResult {
  ...
  const decisions = input.candidates.map((candidate): KaironCandidateDecision => {
    ...
    const governedQuorum = evidenceQuorum(automaticL2Evidence(candidate.evidence));
    const immune = immuneFor(candidate.immuneSignature, input.immunePatterns);
    const authorityStartKnown = input.authorityGraph.some((node) => node.key === candidate.authorityNodeKey);
    const authority = deterministicAuthorityTraversal(input.authorityGraph, candidate.authorityNodeKey, {
      maxDepth: 12,
      maxPaths: 200000,
    });
    const authoritySafe = authorityStartKnown && authority.forbiddenPaths.length === 0;
    ...
    if (!authoritySafe) {
      ...
      reasons.push(authorityStartKnown
        ? "Deterministic authority traversal reaches a forbidden/L3 path."
        : "Authority node is absent from the governed graph; unknown authority fails closed.");
    }
    ...
    if (candidate.quorumRequired && !governedQuorum.reached && decision === "operate") {
      decision = "recommend";
      reasons.push("Independent governed-evidence quorum is not satisfied; low-authority inference cannot vote an action into L2.");
    }
    ...
    if (candidate.domain === "economic" && decision === "operate" && !economicGate.enabled) {
      decision = "recommend";
      reasons.push(`Economic L2 gate is closed: ${economicGate.reason}.`);
    }
    ...
    return {
      ...
      quorumReached: governedQuorum.reached,
      independentSources: governedQuorum.independentSources,
  ...
  const selection = hardVetoSelection(decisions.map((decision) => {
    const nonAutonomous = decision.decision === "abstain" || decision.decision === "escalate";
    return {
      ...
      hardVeto: nonAutonomous,
      vetoReasons: nonAutonomous ? decision.reasons : [],
    };
  }));
