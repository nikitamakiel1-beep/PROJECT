...
import { AlertTriangle, ArrowLeft, CheckCircle2, Cloud, Database, GitCommitHorizontal, RefreshCw, ShieldCheck, Timer, XCircle } from "lucide-react";
...
type Probe = {
  ...
  configuration?: {
    schedulerBindingsConfigured?: boolean;
    schedulerBindingsPresent?: number;
    schedulerBindingsRequired?: number;
    httpControlBindingsConfigured?: boolean;
    httpControlBindingsPresent?: number;
    httpControlBindingsRequired?: number;
  ...
  truth?: {
    ...
    schedulerConfigurationVerified?: boolean;
    databaseReachabilityVerified?: boolean;
    schedulerExecutionVerified?: boolean;
    exactCommitVerified?: boolean;
    httpControlReady?: boolean;
...
function RuntimeDiagnostics() {
  ...
  const schedulerConfigured = p?.truth?.schedulerConfigurationVerified;
  ...
  const httpControlReady = p?.truth?.httpControlReady;
  const ownerControlReady = p?.truth?.ownerControlReady;
  ...
  const blockerList = useMemo(() => {
    ...
    if (!probe.url) blockers.push("The public Worker URL is not configured in the cockpit, so direct /diagz probing cannot run here yet.");
    if (probe.error) blockers.push(`Direct Worker probe failed: ${probe.error}`);
    if (p && p.configuration?.schedulerBindingsConfigured === false) blockers.push("Scheduler bootstrap is missing SUPABASE_SERVICE_ROLE_KEY. SUPABASE_URL is versioned configuration and does not need a secret entry.");
    if (p && p.database?.reachable === false) blockers.push(`Cloudflare cannot reach/authenticate Supabase (${p.database.state ?? "unknown"}).`);
    if (!heartbeatFound) blockers.push("No runtime heartbeat is recorded in Supabase; five-minute scheduled execution is not proven.");
    ...
    if (p && p.configuration?.httpControlBindingsConfigured === false) blockers.push("HTTP control is not fully configured: CRON_SECRET and/or CREIXEMENT_API_TOKEN are missing. This does not block internal heartbeat execution.");
    if (p && p.configuration?.ownerBindingConfigured === false) blockers.push("Owner-control API is not configured: CREIXEMENT_OWNER_TOKEN is missing. This does not block internal heartbeat execution.");
  ...
  return <div className="rd-page">
    ...
    <div className="rd-wrap">
      ...
      <p className="rd-lead">This surface separates deployment, scheduler bootstrap, database reachability, heartbeat execution, HTTP control and owner authority. A green build is never represented as an active autonomous runtime until a fresh heartbeat and exact release evidence exist.</p>
      ...
      <div className="rd-grid">
        <Card icon={<Cloud size={17}/>} label="Worker response" value={workerResponding ? "Responding" : probe.url ? "Unverified" : "URL not configured"} state={truth(workerResponding)} detail={probe.loading ? "Running safe diagnostic probe…" : probe.error || "HTTP liveness is separate from scheduled execution."}/>
        <Card icon={<Database size={17}/>} label="Supabase path" value={databaseReachable ? "Reachable" : "Not verified"} state={truth(databaseReachable)} detail={p?.database?.state ? `Worker reports: ${p.database.state}` : heartbeats.error || "Scheduler bootstrap requires only Supabase URL + server key."}/>
      ...
      <div className="rd-mini-grid">
        <Card icon={<Timer size={17}/>} label="Scheduler bootstrap" value={schedulerConfigured === true ? "Configured" : schedulerConfigured === false ? "Incomplete" : "Unknown"} state={truth(schedulerConfigured)} detail="SUPABASE_URL is versioned. SUPABASE_SERVICE_ROLE_KEY is the only secret required to bootstrap internal heartbeat execution."/>
        <Card icon={<ShieldCheck size={17}/>} label="HTTP control" value={httpControlReady === true ? "Ready" : httpControlReady === false ? "Incomplete" : "Unknown"} state={truth(httpControlReady)} detail="CRON_SECRET and CREIXEMENT_API_TOKEN protect control endpoints; they do not gate the internal scheduler."/>
        <Card icon={<ShieldCheck size={17}/>} label="Owner control" value={ownerControlReady === true ? "Ready" : ownerControlReady === false ? "Incomplete" : "Unknown"} state={truth(ownerControlReady)} detail="CREIXEMENT_OWNER_TOKEN gates consequential owner-decision APIs and never expands L2 authority automatically."/>
      </div>
      ...
      <div className="rd-cols">
        ...
        <section className="rd-panel"><h2>Evidence snapshot</h2><table className="rd-table"><tbody>
          ...
          <tr><td>Scheduler secrets required</td><td>1</td></tr>
        ...
        </tbody></table><p className="rd-note">Browser access uses only publishable Supabase credentials. Service-role, cron, owner and API token values are never exposed here or returned by /diagz.</p></section>
      </div>
    </div>
  </div>;
}
