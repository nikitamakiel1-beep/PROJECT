...
def run_once(path: Path):
    ...
    return run_controlled_microcandidate_lane(path, seed=129)
...
def main() -> None:
    ...
    try:
        ...
        first = run_once(first_root)
        second = run_once(second_root)
        ...
        require(first["summary"]["version"] == "autonomous-microcandidate-lane-v2", "corrected control version missing")
        ...
        for name, candidate in candidates.items():
            ...
            require(evidence["metrics"]["expected_calibration_error"] <= 0.10, f"{name} calibration gate failed")
            ...
            require((first_root / evidence["rollback_artifact"]).exists(), f"{name} rollback artifact missing")
        ...
        print(json.dumps({
            ...
            "metrics": {
                "lora_perplexity": lora["metrics"]["test_perplexity"],
                "embedding_mrr": embedding["metrics"]["retrieval_mrr"],
                "vision_map50": vision["metrics"]["map50"],
            },
